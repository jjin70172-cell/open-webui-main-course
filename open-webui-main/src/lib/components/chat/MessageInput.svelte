<script lang="ts">
	...
	const quickPrompts = [
		{ label: '总结内容', text: '请总结以下内容，并列出3个关键结论：' },
		{ label: '翻译为中文', text: '请将以下内容翻译成中文，保持原意和专业术语准确：' },
		{ label: '优化表达', text: '请优化以下内容的表达，使其更加清晰、简洁和专业：' }
	];

	const applyQuickPrompt = async (text: string) => {
		const currentContent = prompt?.trim() ? prompt : '';
		await setText(currentContent ? `${text}\n${currentContent}` : text);
	};
			...
			>
				<div class="">
					...
					>
						...
						<div class="flex flex-wrap items-center gap-1.5 px-1.5">
							{#each quickPrompts as quickPrompt}
								<button
									type="button"
									class="text-xs px-2.5 py-1 rounded-full border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition"
									on:click={() => applyQuickPrompt(quickPrompt.text)}
								>
									{quickPrompt.label}
								</button>
							{/each}
										...
										{#key $settings?.richTextInput ?? true}
											{#key $settings?.showFormattingToolbar ?? false}
												<RichTextInput
													...
													placeholder={placeholder ? placeholder : $i18n.t('输入消息…')}
